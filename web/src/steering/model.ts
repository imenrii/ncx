import type { DimensionSelection, Metadata, Variable, Probe } from "../data/model.ts";
...
export interface ProbeSelection extends ProbeQuery { source: string; path: string }
...
export type Intent =
  | { kind: "default" }
  | { kind: "hidden" }
  | { kind: "data"; binding?: Binding; error?: string };
export interface ObjectDescription { kind: string; summary: string; fields: { name: string; value: string }[]; target?: PlotTarget; reference?: string; objectId?: string }
...
export type Displays = Record<PlotTarget, DisplayState>;
export interface LogEntry { id: number; kind: "command" | "output" | "error" | "result"; text: string; object?: ObjectDescription; error?: ConsoleError }
